...
@Table(name = "T_ERROR")
@Comment("差错")
public class ErrorDO extends AbstractBaseDO {
	
	private static final long serialVersionUID = 8384383309608180637L;
	...
	@ManyToMany
	@JoinTable(name = "T_ERROR_INSECURITY", joinColumns = @JoinColumn(name = "error_id"), inverseJoinColumns = @JoinColumn(name = "insecurity_id"))
	@Comment("不安全状态")
	...
	@ManyToMany
	@JoinTable(name = "T_CONTROL_ERROR", joinColumns = @JoinColumn(name = "error_id"), inverseJoinColumns = @JoinColumn(name = "control_id"))
	@Comment("控制措施")
	...
	public boolean equals(Object obj) {
		...
		final ErrorDO error = (ErrorDO) obj;
		if (this.getId().equals(error.getId())) {
			return true;
		}
		return false;
	}
