...
public class SSOClient {
	...
	public static String getTicket(final String server, final String username, final String password,
			final String service, final String tgc) {
		...
		if (tgc == null) {
			...
			return getServiceTicket(server, tgc, service);
		}
	}
		...
		try {
			...
			if (200 == post.getStatusCode()) {
				...
				if (response.contains("<cas:authenticationSuccess>")) {
					Pattern pattern = Pattern.compile("<cas:user>(.*?)</cas:user>");
