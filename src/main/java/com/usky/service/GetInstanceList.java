...
public class GetInstanceList extends WfService {
	@Override
	...
	protected boolean OnSmsWork(DbClient dc) throws Exception {
        ...
        String sql = "select a.* from (select wi_id,wt_id,status,title,brief,start_time,end_time,applyer,last_user,remark description,last_update from " + WfMng.db_name + "workflow_inst wi) a where 1 = 1";
