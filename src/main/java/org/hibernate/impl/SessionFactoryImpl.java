...
public final class SessionFactoryImpl implements SessionFactory, SessionFactoryImplementor {
	...
	public SessionFactoryImpl(Configuration cfg, Mapping mapping, Settings settings, EventListeners listeners, SessionFactoryObserver observer) throws HibernateException {
		...
		// modified by steven
		this.classMetadata = classMeta;//Collections.unmodifiableMap(classMeta);
		...
		// modified by steven
		collectionRolesByEntityParticipant = tmpEntityToCollectionRoleMap;//Collections.unmodifiableMap(tmpEntityToCollectionRoleMap);
	...
	private SessionImpl openSession(Connection connection, boolean autoClose, long timestamp, Interceptor sessionLocalInterceptor) {
		return new SessionImpl(connection, this, autoClose, timestamp, sessionLocalInterceptor == null ? interceptor : sessionLocalInterceptor, settings.getDefaultEntityMode(), settings.isFlushBeforeCompletionEnabled(), settings.isAutoCloseSessionEnabled(), settings.getConnectionReleaseMode());
	}
	...
	public org.hibernate.classic.Session openTemporarySession() throws HibernateException {
		return new SessionImpl(null, this, true, settings.getRegionFactory().nextTimestamp(), interceptor, settings.getDefaultEntityMode(), false, false, ConnectionReleaseMode.AFTER_STATEMENT);
	}
	...
	public Reference getReference() throws NamingException {
		...
		return new Reference(SessionFactoryImpl.class.getName(), new StringRefAddr("uuid", uuid), SessionFactoryObjectFactory.class.getName(), null);
	}
	...
	private class CacheImpl implements Cache {
		...
		public boolean containsEntity(String entityName, Serializable identifier) {
			...
			return p.hasCache() && p.getCacheAccessStrategy().getRegion().contains(buildCacheKey(identifier, p));
		}
		...
		public void evictEntity(String entityName, Serializable identifier) {
			...
			if (p.hasCache()) {
				if (log.isDebugEnabled()) {
					log.debug("evicting second-level cache: " + MessageHelper.infoString(p, identifier, SessionFactoryImpl.this));
				}
		...
		public boolean containsCollection(String role, Serializable ownerIdentifier) {
			...
			return p.hasCache() && p.getCacheAccessStrategy().getRegion().contains(buildCacheKey(ownerIdentifier, p));
		}
		...
		public void evictCollection(String role, Serializable ownerIdentifier) {
			...
			if (p.hasCache()) {
				if (log.isDebugEnabled()) {
					log.debug("evicting second-level cache: " + MessageHelper.collectionInfoString(p, ownerIdentifier, SessionFactoryImpl.this));
				}
	 ...
	 * @throws ClassNotFoundException indicates problems reading back serial
	 *             data stream
	 */
	...
	public void rebuild() {
		...
		while (classes.hasNext()) {
			...
			if (!model.isInherited()) {
				IdentifierGenerator generator = model.getIdentifier().createIdentifierGenerator(configuration.getIdentifierGeneratorFactory(), getSettings().getDialect(), getSettings().getDefaultCatalogName(), getSettings().getDefaultSchemaName(), (RootClass) model);
				identifierGenerators.put(model.getEntityName(), generator);
			}
		}
		...
		String cacheRegionPrefix = getSettings().getCacheRegionPrefix() == null ? "" : getSettings().getCacheRegionPrefix() + ".";
		
		Map tmpEntityPersisters = new HashMap();
		Map entityAccessStrategies = new HashMap();
		Map tmpClassMetadata = new HashMap();
		
		classes = configuration.getClassMappings();
		
		while (classes.hasNext()) {
			
			PersistentClass model = (PersistentClass) classes.next();
			
			model.prepareTemporaryTables(mapping, getSettings().getDialect());
			String cacheRegionName = cacheRegionPrefix + model.getRootClass().getCacheRegionName();
			
			// cache region is defined by the root-class in the hierarchy...  
			EntityRegionAccessStrategy accessStrategy = (EntityRegionAccessStrategy) entityAccessStrategies.get(cacheRegionName);
			if (accessStrategy == null && getSettings().isSecondLevelCacheEnabled()) {
				AccessType accessType = AccessType.parse(model.getCacheConcurrencyStrategy());
				if (accessType != null) {
					
					log.trace("Building cache for entity data [" + model.getEntityName() + "]");
					EntityRegion entityRegion = getSettings().getRegionFactory().buildEntityRegion(cacheRegionName, properties, CacheDataDescriptionImpl.decode(model));
					accessStrategy = entityRegion.buildAccessStrategy(accessType);
					entityAccessStrategies.put(cacheRegionName, accessStrategy);
					allCacheRegions.put(cacheRegionName, entityRegion);
				}
		...
		//Named Queries:
		namedQueries.putAll(configuration.getNamedQueries());
		namedSqlQueries.putAll(configuration.getNamedSQLQueries());
		sqlResultSetMappings.putAll(configuration.getSqlResultSetMappings());
		imports.putAll(configuration.getImports());
		...
		Iterator collections = configuration.getCollectionMappings();
		...
		while (collections.hasNext()) {
			...
			CollectionPersister persister = PersisterFactory.createCollectionPersister(configuration, model, accessStrategy, this);
