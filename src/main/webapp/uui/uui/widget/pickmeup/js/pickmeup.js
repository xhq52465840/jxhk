    ...
    var views = {
		...
		tpl = {
		    ...
		    days: function (days) {
		        var result = '';
		        for (var i = 0; i < 42; ++i) {
		            result += '<div class="' + days[i].class_name + ' pmu-button" real="' + days[i].real + '">' + days[i].text + '</div>'
		        }
    ...
    function fill() {
        ...
        for (var i = 0; i < options.calendars; i++) {
            ...
            if (pickmeup.hasClass('pmu-view-years')) {
                ...
                header = formatDate(date, options.title_format, options.locale);
            }
            instance
				.find('.pmu-month')
				.text(header);
            ...
            while (count < 42) {
                day = {
                    text: date.getDate(),
                    class_name: [],
                    real: formatDate(date, options.format, options.locale)
                };
                    ...
                    switch (options.mode) {
                        case 'multiple':
                            ...
                            if (!(options.date instanceof Array)) {
                                var tmp = options.date;
                                options.date = [];
                                options.date.push(tmp);
                            }
                        ...
                        case 'range':
                            ...
                            val = (current_date.setHours(0, 0, 0, 0)).valueOf();
                            var dtend = new Date(options.lastSel);                            
                            if (val < options.lastSel) {
                                for (var t = val ; t <= options.lastSel ; t += 86400000) {
                                    options.date.push(t);
                                }
                            }
                            else {
                                for (var t = options.lastSel ; t <= val ; t += 86400000) {
                                    options.date.push(t);
                                }
                            }
                            options.lastSel = val;
