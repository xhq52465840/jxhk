//@ sourceURL=com.audit.secondaryquery.rectificationquery.TargetProp
$.u.define("com.audit.secondaryquery.rectificationquery.TargetProp", "com.sms.plugin.search.baseprop",{
    ...
    table_html: function (data, type, row, meta) {
    	if(data && data.targetName){
    		return  '<span style="padding-left:15px;">'+(data.targetName?data.targetName:"")+'</span>';
    	}
	},
    ...
    filter_render: function (setting,toolsel) {
    	this._liTemplate = '<li class="list-group-item" >'
									...
									+'<label style="width:100%;">'
										...
										/*+'<span><img style="margin-right: 4px;" height="0" width="0" align="absmiddle" src="#{url}" />#{name}</span>'*/
										+'<span>#{name}</span>'
									+'</label>'
								+'</div>'
							+'</li>';
    ...
    _loadData:function(){
    	...
    	$.u.ajax({
            ...
            type:"post",
            data: {
        		"tokenid":$.cookie("tokenid"),//JSON.parse($.cookie("uskyuser")).tokenid,
    			"method": "getTargetForImproveIssue",
    			"type":"SUB3"
        		//"rule": JSON.stringify([[{"key":"type","value":"当前审计库版本"}]]),
        		//"unitName": term
        	}
        	...
        	if (response.success) {        		
        		...
        		this.dataArray = $.map(response.data.aaData, this.proxy(function(item, idx){
        			return { "id": item.id, "name": item.name/*, "url": this.getabsurl(item.avatarUrl)*/ };
        		}));
