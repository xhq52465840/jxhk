//@ sourceURL=com.audit.plugin.reportquery.columnsProp
$.u.define('com.audit.plugin.reportquery.columnsProp', null, {
              		...
              		$.each(this._userFilter.columns, this.proxy(function(idx, item){
              			...
              			if(item.propid){
              				if($.inArray(item.propid, this.checkedIdArray) < 0){
                  				this.uncheckedArray.push({ "key": item.propid, "name": item.propname, "propvalue": [], "propplugin": item.propplugin });
                  			}
              			}
              		})); 
                      if(this.uncheckedArray.length){
...
com.audit.plugin.reportquery.columnsProp.widgetjs = [];
com.audit.plugin.reportquery.columnsProp.widgetcss = [];