//@ sourceURL=com.sms.dash.staffRptCntByDeptPie
$.u.define('com.sms.dash.staffRptCntByDeptPie', null, {
	...
	_getData : function(){
		$.u.ajax({
			...
			if (response.success) {
				this._reloadStaffRptCntByDeptPie(response.data.nameData, response.data.seriesData);
			}
		})).fail(this.proxy(function(jqXHR, errorText, errorThrown) {

		}));
    },
    _reloadStaffRptCntByDeptPie:function(nameData, seriesData) {
    	var option = {
    		    title : {
    		    	text: '员工报告数量（按部门）',
    		    	x:'center'
		...
		require([ 'echarts', 'echarts/chart/pie', 'echarts/chart/funnel', 'echarts/config' ],
            function (ec) {
                // --- 饼图 ---
                var myChart2 = ec.init(document.getElementById('staffRptCntByDeptPie'));
...
com.sms.dash.staffRptCntByDeptPie.widgetjs = [
...
com.sms.dash.staffRptCntByDeptPie.widgetcss = [{ path: '../../../uui/widget/jqdatatable/css/jquery.dataTables.css' }];