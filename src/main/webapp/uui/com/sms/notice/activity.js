//@ sourceURL=com.sms.notice.activity
$.u.define('com.sms.notice.activity', null, {
    ...
    afterrender: function (bodystr) {
    	this.i18n = com.sms.notice.activity.i18n;
    	...
    	this.collapse_role1_input = this.qid("collapse-role1-input");
    	this.collapse_role2_input = this.qid("collapse-role2-input");
    	this.collapse_role3 = this.qid("collapse-role3");
    	this.org_tree = null;
    ...
    on_collapse_show: function(e){
    	...
    	switch(type){
	    	...
	    	case "role":
	    		...
	    		break;
	    	case "role2":
	    		if(this.collapse_role1_input.hasClass("select2-offscreen") === false){
	    			this._initRole();
	    		}
	    		break;
    	}
    ...
    on_searchuser_click: function(e){
    	...
    	this._ajax(
    		...
    		true,
    		{
    			...
    			dataobject: "user",
    			rule: JSON.stringify([[{"key":"fullname","op":"like","value":value},{"key":"username","op":"like","value":value}]])
    		},
    		...
    		this.proxy(function(response){
            	...
            	$.each(response.data.aaData,this.proxy(function(idx,item){
            		$('<option value="'+item.id+'" data-type="user" title="'+item.fullname+'">'+item.fullname+"("+item.username+")"+'</option>').appendTo(this.col_one_user_select);
            	}));
    		})
    	);
    },
    	...
    	switch(type){
	    	...
	    	case "orga":
	    		...
	    		$.each(nodes, this.proxy(function(idx, node){
	    			var $option = $('<option value="' + node.id + '" data-type="orga" style="color:red;" title="'+node.name+'"></option').text(node.name);
	    	...
	    	case "role"://先循环role，在内部循环安监机构，然后做append
	    		...
	    		if(roleData.length){
	    			...
	    			if(unitData.length){
	    				$.each(roleData, this.proxy(function(idx, data){
	    					var $option = $('<option value="' + data.id + '" data-type="role" style="color:#1DE420;" title="'+data.name+'"></option').text(data.name + "：");
	    		...
	    		break;
	    	case "role2"://先循环role，在内部循环安监机构，然后做append
    			var data = $('option:selected', this.collapse_role3);
    			if(data.length){
    				data.each(this.proxy(function(k, v){
		    			var $option1 = $(v).clone(true), val = $option1.val();
		    			if(!this.all_result.find('option[value=' + v.value + '][data-type=role3]').length){
		    				$option1.appendTo(this.all_result);
		    			}
		    		}));
    			}else{
    				$.u.alert4$.error("未选择用户", $("<div style='padding:0px;margin-bottom: 0px;'/>").appendTo(this.qid("select-error")), 3000);
    			}
	    		break;
	    	default:
	    		$.u.alert4$.error("未选择任何选项", $("<div style='padding:0px;margin-bottom: 0px;'/>").appendTo(this.qid("select-error")), 3000);
    ...
    on_dialog_close: function(e){
    	this.all_result.add(this.col_one_user_select).add(this.collapse_role).add(this.collapse_role3).empty();
    	this.col_one_user_input.add(this.info_theme).add(this.info_content).val("");
    	this.collapse_role_input.select2("data","");
    	this.collapse_role1_input.select2("data","");
    	this.collapse_role2_input.select2("data","");
        this.qid("info-attachment").uploadify('cancel', '*', true);
    	if(this.org_tree){
    ...
    _getRole: function(){
    	this._ajax(
    		...
    		this.proxy(function(response){
            	...
            	$.each(response.data.aaData, this.proxy(function(idx, item){
            		$('<option value="' + item.id + '" data-type="unit" title="'+item.name+'">' + item.name + '</option>').appendTo(this.collapse_role);
            	}));
    		})
    	);
    },
    ...
    _initRole : function(){
    	this.collapse_role1_input.select2({
			multiple: true,
        	ajax:{
	        	url: $.u.config.constant.smsqueryserver,
	            dataType: "json",
		...
		}).on("change", this.proxy(this._role));
    	this.collapse_role2_input.select2({
			multiple: true,
        	ajax:{
	        	url: $.u.config.constant.smsqueryserver,
	            dataType: "json",
    ...
    _role: function(){
    	this.collapse_role3.empty();
    	if(this.collapse_role1_input.val() && this.collapse_role2_input.val()){
    		var roleIds = $.map(this.collapse_role1_input.select2("val"),function(k){
    			return parseInt(k);
    		}),
    		unitIds = $.map(this.collapse_role2_input.select2("val"),function(k){
    			return parseInt(k);
    		});
    		this._ajax(
	    		$.u.config.constant.smsqueryserver,
	    		true,
	    		{
	    			"method": "getUsersByUnitIdsAndRoleIds",
	    			"unitIds": JSON.stringify(unitIds),
	    			"roleIds": JSON.stringify(roleIds)
	    		},
	    		this.collapse_role3.parent(),
	    		{},
	    		this.proxy(function(response){
	            	$.each(response.data.aaData, this.proxy(function(idx, item){
	            		$('<option value="' + item.id + '" data-type="role3" title="'+item.fullname+'(' + item.username +')'+'">' + item.fullname+'(' + item.username +')'+ '</option>').appendTo(this.collapse_role3);
	            	}));
	    		})
	    	);
    	}
    ...
    destroy: function () {
    	...
    	this.qid("info-attachment").uploadify('destroy');
...
com.sms.notice.activity.widgetjs = ['../../../uui/widget/jqztree/js/jquery.ztree.all-3.5.js',
                                  '../../../uui/widget/select2/js/select2.min.js',
                                  "../../../uui/widget/spin/spin.js",
                                  "../../../uui/widget/jqblockui/jquery.blockUI.js",
                                  "../../../uui/widget/ajax/layoutajax.js",
                                  "../../../uui/widget/uploadify/jquery.uploadify.js"];
com.sms.notice.activity.widgetcss = [{id:"ztreestyle",path:"../../../uui/widget/jqztree/css/zTreeStyle/zTreeStyle.css"},
