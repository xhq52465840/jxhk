//@ sourceURL=com.sms.system.treeDictionary_dialog
$.u.define('com.sms.system.treeDictionary_dialog', null, {
    init: function (options) {
    	
    },
    afterrender: function (bodystr) {
    	this.form=this.qid("form");
    	this.dictionary_F=this.qid("dictionary_F");
    	this.dictionary_T=this.qid("dictionary_T");
    	...
    	/*this.father.on("click",this.proxy(function(e){
			...
			this.treeContent.css({left:offset.left-300 + "px",
   ...
   /* onBodyDown:function(e){
    ...
    on_formDialog_create:function(){
    	rule=[[{"key":"type","op":"like","value":"缺陷类型"}]];
			   this.dictionary_F.select2({
		           width: '100%',
		           multiple: false,
		           ajax: {
    ...
    */},
    open:function(param){
        if(param && param.mode=='edit'){
	        ...
	        this.dictionary_F.select2("data",{id:data.dictionaryId,name:data.dictionary});
	        this.dictionary_T.select2("data",{id:data.parentId,name:data.parent});
	        this.qid("leavel").val(data.grade);
         };
    ...
    on_ok_click:function(param){
    	this.form = this.qid("form");
    	if(this.dictionary_F.select2("val")=="" || this.dictionary_T.select2("val")==""){
    		alert("请选择字典名称");
    		return false
    ...
    getFormData:function(){
    	if(this.dictionary_F.select2("val")=="" || this.dictionary_T.select2("val")==""){
    		...
    		return {
    		    grade:this.qid("leavel").val(),
    		    dictionary:Number(this.dictionary_F.select2("val")),
    		     parent:Number(this.dictionary_T.select2("val")),
    ...
    */
    ...
    $(":text,textarea,select",this.form).val("");
    	this.dictionary_F.select2("data",null);
    	this.dictionary_T.select2("data",null);
    }
}, { usehtm: true, usei18n: false });

com.sms.system.treeDictionary_dialog.widgetjs = ["../../../uui/widget/uploadify/jquery.uploadify.js",
...
com.sms.system.treeDictionary_dialog.widgetcss = [
