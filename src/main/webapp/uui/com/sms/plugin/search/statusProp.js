//@ sourceURL=com.sms.plugin.search.statusProp
$.u.define("com.sms.plugin.search.statusProp", "com.sms.plugin.search.baseprop",{
    // filter段
    filter_html: function () {
    	return  '<div class="filter-layer">'
				...
				+ 				'<input type="text" placeholder="查找状态..."  class="form-control input-sm" id="'+this._id+'-search"/>'
    	...
    	if(items && container){
	    	...
	    	$.each(items, this.proxy(function(idx, item){
	        	htmls = this._liTemplate.replace(/#\{id\}/g, item.id)
										...
										.replace(/#\{category\}/g, this._getLabel(item.category));
				$(htmls).appendTo(container).find(":checkbox").data("data", item);
	        }));
    	}
    },
    _getLabel: function(v){
    	var label = null;
    	switch(v){
