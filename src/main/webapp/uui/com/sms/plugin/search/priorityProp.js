//@ sourceURL=com.sms.plugin.search.priorityProp
$.u.define("com.sms.plugin.search.priorityProp", "com.sms.plugin.search.baseprop",{
    // filter段
    filter_html: function () { 
    	return  '<div class="filter-layer">'
				...
				+ 				'<input type="text" placeholder="查找优先级..."  class="form-control input-sm" id="'+this._id+'-search"/>'			
    ...
    filter_render: function (setting,toolsel) {
        ...
        this.priorityUl = $("#"+this._id+"-project-ul",this.filtersel); 
        ...
        this.priorityUl.off("mouseenter","li.list-group-item:not(li.no)").on("mouseenter","li.list-group-item:not(li.no)",this.proxy(function(e){
        	this.priorityUl.find("li.list-group-item").css({"background-color":"#fff"});
        	$(e.currentTarget).css({"background-color":"#ebf2f9"});
        }));
        
        this.priorityUl.off("click", "a.clear-all").on("click", "a.clear-all", this.proxy(function(e){
        	e.preventDefault();
        ...
        this.priorityUl.off("click", ":checkbox").on("click", ":checkbox", this.proxy(function(e){
    	...
    	$.u.ajax({
        	...
        	if (response.success) {        		
        		if(this.setting.propvalue.length>1){
        			$('<li class="list-group-item no" ><a class="clear-all" href="#" id="'+this._id+'-btn_clear">清除选择的内容</a></li>').appendTo(this.priorityUl);	
        		}
                ...
                this._drawItems(this.dataArray, this.priorityUl);
                this.priorityUl.find("li.list-group-item:not(li.no)").eq(0).css({"background-color":"#ebf2f9"});
            }
    ...
    _setData:function(ads){
    	...
    	if(ads){
    		...
    		if(tempArray.length == 0){
    			$('<li  class="list-group-item" style="font-style: italic;">没有匹配的</li>').appendTo(this.priorityUl);
    		}
    	}else if(this.checkedIdArray.length > 1){
    		$('<li class="list-group-item no" ><a class="clear-all" href="#" id="'+this._id+'-btn_clear">清除选择的内容</a></li>').appendTo(this.priorityUl);
    	}
    	this._drawItems(tempArray, this.priorityUl);
        this.priorityUl.find("li.list-group-item:not(li.no)").eq(0).css({ "background-color": "#ebf2f9" });
    },
