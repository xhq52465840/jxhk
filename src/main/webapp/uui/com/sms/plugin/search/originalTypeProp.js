//@ sourceURL=com.sms.plugin.search.originalTypeProp
$.u.define("com.sms.plugin.search.originalTypeProp", "com.sms.plugin.search.baseprop",{
    // filter段
    	...
    	$.u.ajax({
        	...
        	if (response.success) {        		
        		...
        		this.dataArray = $.map(response.data.aaData, this.proxy(function(item, idx){
        			return { "id": item.name, "name": item.name, "url": this.getabsurl(item.url) };
        		}));
