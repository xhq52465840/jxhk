// @ sourceURL=com.sms.dash.error_pie
$.u.define('com.sms.dash.error_pie', null, {
	...
	afterrender : function(bodystr) {
			...
			ajax : {
				...
				data : this.proxy(function(term, page) {
					return {
						...
						rule : JSON.stringify([[{'key' : 'name', 'op' : 'like', 'value' : term} ],[{"key": "system", "value": this.param && this.param.system !== "0" ? parseInt(this.param.system) : null}]]),
						start: (page - 1) * this._select2PageLength,
						length: this._select2PageLength,
		...
		this._ajax($.u.config.constant.smsqueryserver, true, {
			...
			if (this._initmode == 'config') {
				...
				//this._getData({obj : JSON.stringify({'unitId' : this.unitId, 'insecurityId' : '', 'systemId' : this.systemId, paramType : 'error'})});
				this.qid("search").click(this.proxy(function(){
					var insecurityId = this.qid("insecurity").val()=='0'?"":this.qid("insecurity").val();
					this._getData({obj : JSON.stringify({'unitId' : this.unitId, 'systemId' : this.systemId, 'insecurityId' :insecurityId, paramType : 'error'})});
				})).trigger("click");
			}
		}));
		...
		this.qid("cancel").click(this.proxy(function() {
			...
			this._getData({obj : JSON.stringify({'unitId' : this.unitId, 'insecurityId' : this.insecurityId, 'systemId' : this.systemId, paramType : 'error'})});
		}));
	...
	_loadPie : function(timeData, scoreListOneYear, insecurityIdListOneYear) {
		...
		var $mainPie = this.qid("mainPie")[0];
		require.config({
			paths : {
				echarts : './echarts/js'
			}
		...
		require([ 'echarts', 'echarts/chart/pie','echarts/chart/funnel' ], function(ec) {
			...
			// 图例
			var legend = {orient : 'vertical', x : 'left', y : 'top'};
			var legendData = [];
		...
		$.u.ajax({
			...
			if (data.success) {
				...
				if (window.parent) {
					...
					window.parent.setGadgetTitle(this._gadgetsinstanceid, this._gadgetsinstance.gadgetsDisplayName + "：(安监机构：" + this.unitName + ",　系统分类：" + this.systemName + ")");
				}
			}
...
com.sms.dash.error_pie.widgetjs = [ '../../../uui/widget/select2/js/select2.min.js', '../../../uui/widget/spin/spin.js',
		'../../../uui/widget/jqblockui/jquery.blockUI.js', '../../../uui/widget/ajax/layoutajax.js',
		'echarts/js/echarts.js', '../../../uui/widget/jqurl/jqurl.js' ];
com.sms.dash.error_pie.widgetcss = [{id:"",path:"../../../uui/widget/select2/css/select2.css"},
