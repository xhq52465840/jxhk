//@ sourceURL=com.sms.plugin.search.aircraftTypeCatProp
$.u.define("com.sms.plugin.search.aircraftTypeCatProp", "com.sms.plugin.search.baseprop",{
    // filter段
    	...
    	$.u.ajax({
        	...
        	if (response.success) {
        		...
        		for(i in response.data.aaData){
        			this.dataArray.push({id:response.data.aaData[i],name:response.data.aaData[i]})
        		};
        		/*var arry=[];
        		this.dataArray=[];
        		for(i in  response.data.aaData){
        			if(arry.indexOf(response.data.aaData[i].aircraftType)==-1){
        				this.dataArray.push({"id": response.data.aaData[i].aircraftType, "name": response.data.aaData[i].aircraftType});
        				arry.push(response.data.aaData[i].aircraftType);
        			}
        		}*/
