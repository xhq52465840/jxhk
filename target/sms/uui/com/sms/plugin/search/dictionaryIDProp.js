//@ sourceURL=com.sms.plugin.search.dictionaryIDProp
$.u.define("com.sms.plugin.search.dictionaryIDProp", "com.sms.plugin.search.baseprop",{
    // filter段
    ...
    filter_render: function (setting,toolsel) {
        ...
        this.dictionaryUl = $("#"+this._id+"-project-ul",this.filtersel); 
        this.searchAds = $("#"+this._id+"-search",this.filtersel).focus();
        
        this.searchAds.attr("placeholder", "查找" + this.setting.propname + "...");
        ...
        this.dictionaryUl.off("mouseenter","li.list-group-item:not(li.no)").on("mouseenter","li.list-group-item:not(li.no)",this.proxy(function(e){
        	this.dictionaryUl.find("li.list-group-item").css({"background-color":"#fff"});
        	$(e.currentTarget).css({"background-color":"#ebf2f9"});
        }));
        
        this.dictionaryUl.off("click", "a.clear-all").on("click", "a.clear-all", this.proxy(function(e){
        	e.preventDefault();
        ...
        this.dictionaryUl.off("click", ":checkbox").on("click", ":checkbox", this.proxy(function(e){
    ...
    _loadData:function(){
    	...
    	$.u.ajax({
            ...
            data: {
				...
				"rule": JSON.stringify([[{ "key":"type", "value":this.setting.config }]])
        	}
        }, this.filtersel, {size: 2, backgroundColor: "#fff"}).done(this.proxy(function (response) {
        	if (response.success) {        		
        		if(this.setting.propvalue.length>1){
        			$('<li class="list-group-item no" ><a class="clear-all" href="#" id="'+this._id+'-btn_clear">清除选择的内容</a></li>').appendTo(this.dictionaryUl);	
        		}
                ...
                this._drawItems(this.dataArray, this.dictionaryUl);
                this.dictionaryUl.find("li.list-group-item:not(li.no)").eq(0).css({"background-color":"#ebf2f9"});
            }
    ...
    _setData:function(ads){
    	...
    	if(ads){ 
    		tempArray = $.grep(this.dataArray, function(item, idx){
        		return item.name && item.name.indexOf(ads) > -1;
        	});
    		if(tempArray.length == 0){
    			$('<li  class="list-group-item" style="font-style: italic;">没有匹配的</li>').appendTo(this.dictionaryUl);
    		}
    	}else if(this.checkedIdArray.length > 1){
    		$('<li class="list-group-item no" ><a class="clear-all" href="#" id="' + this._id + '-btn_clear">清除选择的内容</a></li>').appendTo(this.dictionaryUl);
    	}
    	this._drawItems(tempArray, this.dictionaryUl);
        this.dictionaryUl.find("li.list-group-item:not(li.no)").eq(0).css({ "background-color": "#ebf2f9" });
    },
