//@ sourceURL=com.sms.plugin.search.usersProp
$.u.define("com.sms.plugin.search.usersProp", "com.sms.plugin.search.baseprop",{
    ...
    _transformRule: function(s, isImg){
    	var result = s;
    	if(isImg === true && result == "currentUser()"){
        	result = $.parseJSON($.cookie("uskyuser")).avatarUrl;
    	}else{
    		switch(s){
	    		case "currentUser()": result = "当前用户"; break;
	    		// no default
	    	}
    	}
    	return result;
    	...
    	if(items && container){
	    	...
	    	$.each(items, this.proxy(function(idx, item){
	        	htmls = this._liTemplate.replace(/#\{id\}/g, item.id)
										.replace(/#\{name\}/g, this._transformRule(item.name))
										.replace(/#\{checked\}/g, $.inArray(item.id, this.checkedUserIdArray) > -1 ? "checked" : ""  )
										.replace(/#\{url\}/g, this._transformRule(item.url, true));
    ...
    _drawDefaultPage: function(){
		...
		if($.inArray(currUser, this.checkedUserIdArray) < 0){
			tempArray.push({ id: currUser, name: currUser, url: currUser});
		}
    ...
    filter_getdata: function () {
    	...
    	this.setting.propshow = $.map(this.checkedUserArray, this.proxy(function(item, idx){
    		return this._transformRule(item.name);
    	})).join(",");
    	return this.setting;
    },
