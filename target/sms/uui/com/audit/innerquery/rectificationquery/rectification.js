//@ sourceURL=com.audit.innerquery.rectificationquery.rectification
$.u.load("com.sms.common.stdcomponentdelete");
$.u.define('com.audit.innerquery.rectificationquery.rectification', null, {
	//	copy from com.sms.search.searchindex
    init: function () {
    	  this._filtertemplate = "<li data-id='#{propid}' class='criteria-item' style='display:inline-block'>" +
    ...
    afterrender: function () {
    	 ...
    	 */
    	this.i18n = com.audit.innerquery.rectificationquery.rectification.i18n;
    	...
    	$.u.ajax({
        	...
        	if (response.success) {
        		this.targetIDdata=$.map(response.data.aaData,this.proxy(function(item,idx){
    ...
    on_btn_more:function(e){
		...
		moreRenderClazz = $.u.load("com.audit.plugin.rectificationquery.moreProp");
       ...
       /*     
          ...
          this.dataTable=this.qid("viewtable").dataTable({
                ...
                "ajax": this.proxy(function (data, callback, settings) {
                	...
                	if(array_e.length>1 && array_e[0]=="planTime"||array_e[0]=="transactor"){
                		sort=[];
                		array_e[0]="target";
                		array_e[1]="asc";
                	}
        		...
        		//	{"method":"stdcomponent.getbysearch", "dataobject":"item",	"rule": JSON.stringify([[{"key":"type","value":null}]])},
                	...
                	$.each(querya,this.proxy(function(idx,item){
	                	if(item.propvalue){//item={key: "type",propvalue: Array[3]}
	                		$.each(item.propvalue,this.proxy(function(idx,field){
	                			if(item.key=="planTime"&&field.type=="dateRange"){
	                				...
	                				queryc.push({"key":item.key,"value":field.id.toString()});
                				}else if(item.key=="planType"||item.key=="transactorTel"){
	                				queryd.push({"key":item.key,"value":field.id.toString()});
                				}
                				else
                					rulecache.push([{"key":item.key,"op":"like","value":field.name}]);
                	...
                	queryc.length==0?"":(rulecache.push(queryc));
//                    if(queryc.length==0 && this.targetIDdata.length>0){
//						rulecache.push([{"key":"target","op":"in","value":this.targetIDdata.length>0?this.targetIDdata:[null]}]);
//					}
				//	rulecache.push([{"key":"target","op":"in","value":this.targetIDdata.length>0?this.targetIDdata:[null]}]);

                	rulecache.push([{"key":"task.planType","op":"in","value":["SUB2"]}]);
                	rulecache.push([{"key":"permittedQuery","value":true}]); 
                	delete data.order;
                     delete data.draw;
                     delete data.search;
                	 delete data.columns;
                	...
                	this._ajax($.u.config.constant.smsqueryserver,$.extend({},data,{
                		...
                		"rule":JSON.stringify(rulecache) ,//JSON.stringify([[{key:"type","value":null}]]),
                		"dataobject": "improve",
                	/*	"columns":JSON.stringify([{"data":"orderNo"}]),
                   	 	"order":JSON.stringify([{"column":0,"dir":"desc"}]),*/
            	 		//"sort": sort
                		"columns":JSON.stringify([{"data":array_e[0]}]),
            	 		"order":JSON.stringify([{"column":0,"dir":array_e[1]}])
                ...
                "headerCallback": this.proxy(function( thead, data, start, end, display ) {
                	...
                	$(".results th").css("min-width","90px");
                	$(".results th.field-improveNo").css("min-width","130px");
                })
            });
...
com.audit.innerquery.rectificationquery.rectification.widgetjs = ['../../../../uui/widget/jqdatatable/js/jquery.dataTables.js',
...
com.audit.innerquery.rectificationquery.rectification.widgetcss = [{ path: '../../../../uui/widget/jqdatatable/css/jquery.dataTables.css' },
