//@ sourceURL=com.audit.filter.selectProp
$.u.define("com.audit.filter.selectProp", "com.sms.plugin.search.baseprop",{
    // filter段
    filter_html: function () { 
    	return  '<div class="filter-layer">'
				...
				+ 				'<span class="glyphicon glyphicon-search form-control-feedback removeads" style="cursor:pointer;top:0px;"></span>'			
				+ 			'</div>'		
			    + 		'</div>'
				+ 		'<div style="max-height: 200px;overflow-y: auto;overflow-x: hidden;clear: both;">'	
    ...
    filter_render: function (setting,toolsel) {
    	this._liTemplate = '<li class="list-group-item" >'
									...
									+'<label style="width:100%;">'
										+'<input type="checkbox" name="#{name}" value="#{id}" #{checked} />#{name}'
									+'</label>'
								+'</div>'
							+'</li>';
        ...
        this.statusUl.off("click", ":checkbox").on("click", ":checkbox", this.proxy(function(e){
        	...
        	//如果true，checkedIdArray中只有他自己，其他都不选中；如果false，checkedIdArray清空
        	
        	if($this.is(":checked")){
        		//this.checkedIdArray = [];
        		//$this.parents('li').siblings().find(":checked").prop("checked",false);
    ...
    _loadData:function(){
    	...
    	this.dataArray = []; 	
    	this.checkedIdArray = [];
    	if(this.setting.propid=="improveUnit"){
    		operatorsPropvalue=$($(".search-criteria").find('.criteria-item')[1]).data("data-data").propvalue;
    		var mm=[];
    		for(i in operatorsPropvalue){
    			mm[i]=operatorsPropvalue[Number(i)].id;
    		}
    		this.setting.config.operators=JSON.stringify(mm);
    	...
    	$.u.ajax({
            ...
            dataType: "json",
            type:"post",
            data: $.extend({"tokenid": $.cookie("tokenid")}, this.setting.config)
        	...
        	if (response.success) {
        		...
        		if(response.data.aaData){
        			...
        			this.dataArray = $.map(response.data, function(item, idx){
            			return { "id": item.id, "name": item.name};
            		});
        		}
