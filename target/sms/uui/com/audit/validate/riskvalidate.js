  ...
  generateDataTableColumns: function(columnNames){
    var columns = {
      ...
      idConfirm:{
        ...
        defaultContent: "--",
        render: function(data, type, full){
          return "<div></span><input type='checkbox' name='checkall-confirm'  data-data='" + JSON.stringify(full) + "'/></div>";
        }
      },
      idApproval:{
        ...
        defaultContent: "--",
        render: function(data, type, full) {
          return "<div></span><input type='checkbox' name='checkall-approval'  data-data='" + JSON.stringify(full) + "'/></div>";
        }
      },
      description: {
        title: "行动项",
	  ...
	  excel:function(){
		  ...
		  var obj = this.excel_rule;   //导出的查询条件
	        this._exportExcelOptions=$.extend(obj, {
	        	method: 'exportActionItemsToExcel',
	            tokenid: $.cookie('tokenid'),
	            "rule": window.encodeURIComponent(JSON.stringify(obj)),
	            titles: window.encodeURIComponent(JSON.stringify(this.cols)),
	        });
	        var form = $("<form/>"), 
