//@ sourceURL=com.audit.plugin.reportquery.queryNameProp
$.u.define("com.audit.plugin.reportquery.queryNameProp", "com.sms.plugin.search.baseprop", {
    // table段
	//copy form com.sms.plugin.render.summaryProp
	//工作单名称
    table_html: function (data, type,row) { 
    	
    	if(parseInt(row.flowStep)> 3){
        	return "<span><a href='" + this.getabsurl("../../xitong_jihua/Xitong_jihua_shangchuanshenjibaogao.html?id=" + row.id) + "' target='_top'>" + (data || "") + "</a></span>"; 
    	}else if(parseInt(row.flowStep)== 3){
        	return "<span><a href='" + this.getabsurl("../../xitong_jihua/Xitong_jihua_shengchengshenjibaogao.html?id=" + row.id) + "' target='_top'>" + (data || "") + "</a></span>"; 
    	}
