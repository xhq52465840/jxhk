//@ sourceURL=com.audit.plugin.validate.queryStatusProp
$.u.define("com.audit.plugin.validate.queryStatusProp", "com.sms.plugin.search.baseprop",{
    ...
    _loadData:function(){
    	...
    	this.dataArray = []; 	
    	this.checkedIdArray = [];
    	var statusAy=["完成","待执行","待执行(验证拒绝)","待验证",'待执行(审核拒绝)','待审核'];
    	if(this.setting.propvalue!=null && this.setting.propvalue.length>0){
			$('<li class="list-group-item no" ><a class="clear-all" href="#" id="'+this._id+'-btn_clear">清除选择的内容</a></li>').appendTo(this.typeUl);	
		}
		this.checkedIdArray = $.map(this.setting.propvalue!=null && this.setting.propvalue, function(v, k){ 
			return v.id;
		});
		this.dataArray = $.map(statusAy, this.proxy(function(item, idx){
