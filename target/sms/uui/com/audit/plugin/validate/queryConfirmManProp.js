//@ sourceURL=com.audit.plugin.validate.queryConfirmManProp
$.u.define("com.audit.plugin.validate.queryConfirmManProp", "com.sms.plugin.search.baseprop",{
    ...
    filter_html: function () { 
    	return  '<div class="filter-layer">'
				...
				+ 				'<input type="text" placeholder="查找验证人..."  class="form-control input-sm" id="'+this._id+'-search"/>'			
    ...
    filter_render: function (setting,toolsel) {
        ...
        this.searchAds.off("keyup").on("keyup", this.proxy(function(e){
        	...
        	if(ads){
        	    ...
        	    this.PAGE_LENGTH = 10;
        		this._loadData(ads);
        		$(".list-group li:gt(0)").removeClass("hidden");
        ...
        this.typeUl.off("click", "a.moreresult").on("click", "a.moreresult", this.proxy(function(e){
        	e.preventDefault();
        	this.CURR_PAGE++;
        	this._loadData(this.ads);
        }));
    ...
    _drawItems: function(items, container,ads){
    	if(items && container){
    		var htmls = "";
    ...
    _loadData:function(ads){
    	...
    	$.u.ajax({
            ...
            data: {
        		...
        		"unitIds":'['+$(".myUnitID").val()+']',
    			"start": this.CURR_PAGE * this.PAGE_LENGTH,
        		"length": (this.CURR_PAGE + 1) * this.PAGE_LENGTH
        	}
        }, this.filtersel, {size: 2, backgroundColor: "#fff"}).done(this.proxy(function (response) {
        	if (response.success) {
        		if(this.setting && this.setting.propvalue && this.setting.propvalue.length>0){
        		...
        		if(!ads){
        			response.data.aaData=[{"id":$.cookie("userid"),displayName:"当前用户"}];
        		}else{
        			response.data.aaData=response.data.aaData.reverse();
                ...
                if( (this.CURR_PAGE + 1) * this.PAGE_LENGTH < response.data.iTotalRecords && ads){
                	$("<li class='list-group-item text-center'><a class='moreresult'>更多</a></li>").appendTo(this.typeUl);
                }
    ...
    _setData:function(ads){
    	...
    	tempArray && $.each(tempArray,function(idx,item){
    		if(item.id==$.cookie("userid")){
    			status=1;
    		}
    	});
    	if(status==0){
    		tempArray.push({"id":$.cookie("userid"),name:"当前用户"});
    	}
    	this.typeUl.empty();
    	if(ads){
    	...
    	this._drawItems(tempArray, this.typeUl,ads);
        this.typeUl.find("li.list-group-item:not(li.no)").eq(0).css({ "background-color": "#ebf2f9" });
    },
