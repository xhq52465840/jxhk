//@ sourceURL=com.audit.plugin.validate.queryResultProp
$.u.define("com.audit.plugin.validate.queryResultProp", "com.sms.plugin.search.baseprop",{
    	...
    	/*$.u.ajax({
        	...
        	if (response.success) {   		
        		...
        		this.dataArray = $.map(response.data, this.proxy(function(item, idx){
        			return { "id": item.id||item, "name": item.name||item, "url": this.getabsurl(item.avatarUrl) };
        		}));
                this._drawItems(this.dataArray, this.typeUl);
                this.typeUl.find("li.list-group-item:not(li.no)").eq(0).css({"background-color":"#ebf2f9"});
            }
